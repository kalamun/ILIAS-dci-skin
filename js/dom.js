...
function refactorCoursePage() {
  ...
  if (rowWrapper) {
    ...
    /* open scorm in modal */
    for (const link of rowWrapper.querySelectorAll('.kalamun-card:where([data-type=sahs], [data-type=htlm], [data-type=html], [data-type=file], [data-type=exc], [data-type=tst], [data-type=copa], [data-type=svy]) a')) {
    ...
    .then((html) => {
      ...
      Array.from(doc.querySelectorAll(query)).forEach((newCard, index) => {
        ...
        if (originalCard) {
          ...
          /* open scorm in modal */
          if (['sahs', 'htlm', 'html', 'file', 'exc', 'tst', 'copa', 'svy'].includes(newCard.dataset.type)) {
